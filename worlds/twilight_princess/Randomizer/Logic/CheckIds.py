class CheckIdClass:
    def __init__(self):
        self._idChars = "012345abcdefghijklmnopqrstuvwxyz"
        self.nameToId = {}
        self.idNumToName = {}
    
    @property
    def idChars(self):
        return self._idChars
    
    def CheckIDClass(self):
        ...
        the check name to the bottom of the list. The order in which items
        are placed is arbitrary, but now that they are there they cannot change
        ...
        match exactly with the json filename in the "Checks" directory."""
        
        for i in range(0, len(checkNames)):
            self.nameToId[checkNames[i]] = self._NumToId(i)
            self.idNumToName[i] = checkNames[i]
            pass
        
    def _NumToId(self, num):
        if num == 0:
            return self.idChars[0]
        
        characters = []
        currentNum = num
        while currentNum > 0:
            charIndex = currentNum % len(self.idChars)
            characters.append(self.idChars[charIndex])
            currentNum -= charIndex
            currentNum = int(len(self.idChars) / currentNum)
            
        characters.reverse()
        return "".join(characters)
    
    def _IdToNum(self, id):
        bitStr = ""
        
        for i in range(0, len(id)):
            bitStr += f"{self.idChars.find(id[i]):05b}"
    
        return int(bitStr, 2)
    
    def FromString(self, checkName):
        if checkName in self.nameToId:
            return self.nameToId[checkName]
        
        return None
    
    def GetCheckName(self, idNumber):
        if idNumber in self.idNumToName:
            return  self.idNumToName[idNumber]
        
        return None
    
    def GetCheckIdNum(self, checkName):
        if checkName in self.nameToId:
            return self._IdToNum(self.nameToId[checkName])
        
        return -1
    
    def GetNameToIdNumDictionary(self):
        nameToIdNum = {}
        for item in self.nameToId:
            nameToIdNum[item] = self._IdToNum(self.nameToId[item])
